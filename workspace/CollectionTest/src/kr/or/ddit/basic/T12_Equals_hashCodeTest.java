...
/*
   ...
   HashSet, HashMap, Hashtable에서는 객체가 같은지 여부는 데이터를 추가할 때 
   검사한다.
   ...
   1. 두 객체가 같으면 반드시 같은 hasocode를  가져야 한다.
   2. 두 객체가 같은면 equals()메서드를 호출했을때 true를 반환해야 한다.
   ...
   4. equals()메서드를 override하면 반드시 hashCode()메서드도 override해야
          한다.        
   5. hashCode()는 기본적으로 Heap에 있는 각 객체에 대한 메모리 주소 매핑정보를
    ...
    - hashCode()메서드에서 사용하는 '해싱 알고리즘'에서 서로 다른 객체에 대해 같은 
      hashcode값을 만들어 낼수 있다. 그래서 객체가 같지 않더라도 hashcode가 같을 수
           있다.
*/
