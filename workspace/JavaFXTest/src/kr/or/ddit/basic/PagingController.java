...
/*		allTableData.add(new MemberVO("1", "홍길동1", "대전시 중구 대흥동 대덕인재개발원"));
		...
		//                   어레이리스트 전체사이즈%내가설정한 보여줄항목수 == 0.    5로 나눠서 똑 떨어지면 그 나눈값만큼 페이지인덱스 만들고, 0아니면 1개 더 만든단의미.
		pagination.setPageCount(totPageCount); // 전체페이지 수 설정
		
		// 페이징처리의 버튼을 누를때마다 이벤트처리를 어떻게 해줄지
		// 페이지다음다음 누를때마다 새로 그려주는 개념. 1을 누르면 페이지인덱스에0이 넘어가고  
		pagination.setPageFactory(new Callback<Integer, Node>() { //셋페이지팩토리 파라미터는 콜백객체
			
			@Override
			public Node call(Integer pageindex) { //여기에 서 받은 페이디인덱스에따라 새로그그려준다.
				from = pageindex * itemsForPage;
				to = from + itemsForPage -1;
				tableView.setItems(getTableViewData(from, to));
				
				return tableView; // node인건 전부 페이징처리할 수 있다. hbox도 가능. return을 그 페이징 준 node로 하면 된다.
			}
		});
	}
