...
public class T16_DialogTest_보람언니 extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		btnFileOpen.setOnAction(e->{
			...
			if(selectFile != null) {
				...
				System.out.println("OPEN : " 
							+ selectFile.getPath()); //getPath : 경로가져오기
			}
		...
		btnPopup.setOnAction(e4->{
			...
			popup.setX(1000);
			popup.setY(400);
			popup.getContent().add(popRoot); //node는 다 들어감. (팝업이미지와 메시지를 추가함.)
			popup.setAutoHide(true); //false로 하면 다른 행위를 해도 계속 떠있음.
		...
		btnCustom.setOnAction(e->{
			...
			// 2. 모달창 여부 설정 (모달 : 자식창이 열려있는 동안 부모창에 접근할 수 없음)
			//모달창은 자식창이 나타나면 부모창을 사용할 수 없다.
			dialog.initModality(Modality.APPLICATION_MODAL);
			
			// 3. 부모창(primaryStage) 지정
