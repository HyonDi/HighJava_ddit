...
public class T02_FileTest {
	...
	public static void main(String[] args) {
		...
		if(f1.exists()){
			...
			try {
				if(f1.createNewFile()){ //파일이 없으면 새로 만든다
		...
		File f3 = new File("d:/D_Other"); //파일 객체만 만듦
		File[] files = f3.listFiles(); //디렉토리 안에 있는 파일 목록을 하나씩 객체화해서 File 타입의 배열에 담는다
		...
		System.out.println("=========================");
		String[] strFiles = f3.list(); //파일명을 String으로 받는다(객체화X)
	...
	public static void displayFileList(File dir){
		...
		for(int i=0; i<subDirList.size(); i++){
			// 하위 폴더의 내용들도 출력하기 위해 
			// 현재 메서드(displayFileList)를 재귀호출하여 처리한다.
