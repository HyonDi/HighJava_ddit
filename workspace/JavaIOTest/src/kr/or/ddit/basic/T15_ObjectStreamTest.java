...
public class T15_ObjectStreamTest {
	public static void main(String[] args) {
		...
		try {
			...
			// 출력용 스트림 객체 생성
			ObjectOutputStream oos = new ObjectOutputStream( // 보조스트림  
					new BufferedOutputStream( // 보조스트림  
							new FileOutputStream("d:/D_Other/memOBJ.bin"))); // 기반스트림
			
			// 쓰기 작업
			oos.writeObject(mem1); //write 하는 순간에 직렬화가 일어남. 멤버에 seralizable 임플한것 은 직렬화 해도돼. 라고 알려준것뿐. 
			...
			try {
				while((obj = ois.readObject()) != null) { // object를 readobject로 가져옴.
					// 읽어온 데이터를 원래의 객체형으로 변환 후 사용한다.
					Member mem = (Member)obj; // member타입으로 형변환(캐스팅)한다.
			...
			e.printStackTrace();
			// 더이상 읽어올 객체가 없으면 예외 발생. 윗줄 지우면 에러도 안나는것처럼 됨!
...
class Member implements Serializable {
	...
	//				직렬화 : 멤버변수의 값을 seialization 하는 것. 

	private String name;
	private transient int age; // 나이는 serialization 하지 않았음.
	private String addr;
