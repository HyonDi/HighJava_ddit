 ...
 * 4. 각각의 쓰레드는 이 인스턴스의 참조값을 
 * 	    저장한 변수를 이용하여 공통 데이터를 사용한다.
 * 
 * 예) 원주율을 계산하는 쓰레드가 있고, 계산된 원주율을 출력하는 쓰레드가 있다.
 * 		원주율을 계한한 후 이 값을 출력하는 프로그램을 작성하시오.
 * 		(이 때 원주율을 저장하는 객체가 필요하다.)
 *
 *
 *두개의 쓰레드가있음. 하나는 원주율계산.
 *이거를 공유객체에 넣어놓을것이야.
 *이걸 화면에 출력한 쓰레드를 하나 더 만들어. 
 *공유객체에들어있는 그 값을 화면에 출력시킬 쓰레드.
 * @author PC-16
 *
 */
...
// 원주율을 관리하는 클래스(공통으로 사용할 클래스. 공유객체)
class ShareData{
	...
	/*
	 volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외시킨다.
	 			 ...
	 			 보장하는 키워드(일종의 동기화) 발러틸!
	 */
	// 원주율 계산이 완료되었는지를 나타내는 변수.
	/*volatile */public boolean isOK = false;
	
	//volatile 없으면 isOK 가 캐쉬에있는것을 참고하다보니 계속 false. 
	//volatile이 메인메모리에잇는거를 가져오게하는역할임.
}
...
class CalcPIThread extends Thread{
	...
	public void run() {
		...
		double sum = 0.0;
		for(int i = 1; i<=2100000000; i+=2) {// 저 i 범위가 크면 클수록 PI랑 가까워진다.
		...
		sd.isOK = true; //계산이 완료됨을 나타낸다. 이후 이 쓰레드는 종료됨.
	}
}
...
class PrintPIThread extends Thread {
	...
	public void run() {
		while(true) {
			...
			if(sd.isOK) {
				break;
			}// 계산이 끝나 isOK가 true 가 되면 무한루프를 벗어나 아래 sysout부분 실행.
		}
		System.out.println();
